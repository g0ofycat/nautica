...
/// @return Tensor
Tensor Tensor::zeros(const std::vector<size_t> &dims) { return Tensor(dims); }
...
void Tensor::fill(double v) { std::fill(data.begin(), data.end(), v); }
...
double &Tensor::at(const std::vector<size_t> &idx) { return data[index_of(idx)]; }

/// @brief Access tensor element using a vector of indices (const)
/// @param idx The vector of indices
/// @return The tensor element
double Tensor::at(const std::vector<size_t> &idx) const { return data[index_of(idx)]; }
